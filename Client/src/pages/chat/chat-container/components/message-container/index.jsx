...
const MessageContainer = () => {
  ...
  const {selectedChatType , selectedChatData , userInfo , selectedChatMessages , setSelectedChatMessages , setIsDownloading , setFileDownloadProcess }= useAppStore();
...
const downloadFile =async (url) => {
  ...
  setFileDownloadProcess(0);
  const response = await apiClient.get(`${HOST}/${url}`,
    {responseType:"blob" , 
      onDownloadProgress:(progressEvent) => {
        const { loaded , total } = progressEvent;
        const percentCompleted = Math.round((loaded * 100) / total);
        setFileDownloadProcess(percentCompleted);
      }
    });
...
const renderChannelMessages = (message) =>{
  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-hidden p-4 px-8 lg:w-[70vw] md:w-[65vw] xl:w-[80vw] w-full ">
          {renderMessages()}
          <div ref={scrollRef}/>
